# realtime_agent.py
"""
Realtime multi-speaker receptionist:
- Robust RMS VAD (callback + Queue, WASAPI-friendly)
- Speaker labelling via utils.speaker_id (MFCC + cosine)
...
def _flush_print(msg): print(msg); sys.stdout.flush()

def write_wav(path, int16_audio, sr):
    path.parent.mkdir(parents=True, exist_ok=True)
    with wave.open(str(path), "wb") as wf:
        wf.setnchannels(1); wf.setsampwidth(2); wf.setframerate(sr); wf.writeframes(int16_audio.tobytes())
...
def _unique_ints(seq):
    ...
    for x in seq:
        ...
        xi = int(x)
        if xi not in seen: out.append(xi); seen.add(xi)
    return out

def _extract_name_from_text(text: str) -> Optional[str]:
    """
    Capture simple introductions like:
      "I'm Ali", "I am Asraf", "This is John", "My name is Riya"
    """
    t = (text or "").strip()
    m = re.search(r"\b(i am|i'm|this is|my name is)\s+([A-Za-z][A-Za-z\-']{1,30})\b", t, flags=re.I)
    if m:
...
def _open_stream_with_callback(device, preferred_sr, frame_ms):
    ...
    def make_callback(channels):
        def _cb(indata, frames, t, status):
            if status:  # overflow/underflow is non-fatal
                pass
            try:
    ...
    for sr in sr_candidates:
        for cfg in attempt_cfgs:
            try:
                ...
                last_err = e; continue
            try:
                stream = sd.InputStream(samplerate=sr, device=device if device is not None else None,
                                        blocksize=0, dtype=cfg["dtype"], channels=cfg["channels"],
...
def _gather_samples(q, needed, timeout_s):
    ...
    while got < needed and (time.time() - t0) < timeout_s and _running:
        try: x = q.get(timeout=0.2); chunks.append(x); got += len(x)
...
def realtime_loop(device, samplerate, frame_ms, end_silence_ms, max_utterance_sec,
    ...
    # Init speaker ID + memory
    spk_id = SpeakerID(sample_rate=16000, n_mfcc=40, threshold=0.60)  # embeds at 16k internally
    ...
    in_speech = False; utter_frames = []; silence_run = 0; utter_start_ts = None; debounce_until = 0.0
    ...
    noise_ema = noise_r; ema_alpha = 0.05
    ...
    while _running:
        ...
        now = time.time()
        if now < debounce_until:  # cooldown
            continue

        if frame_r > enter_thresh:
            if not in_speech:
                in_speech = True; utter_frames = []; utter_start_ts = now
                if len(prebuf): utter_frames.extend(list(prebuf))
            utter_frames.append(frame.copy()); silence_run = 0
            if (now - (utter_start_ts or now)) > max_utterance_sec:
                in_speech = False
        else:
            if in_speech:
                silence_run += 1; utter_frames.append(frame.copy())
        ...
        if (not in_speech) and utter_frames:
            ...
            if total_samples < int(used_sr * (min_utter_ms / 1000.0)):
                utter_frames = []; silence_run = 0; utter_start_ts = None
            ...
            base_prompt = cfg.get("llm", {}).get(
                "base_system_prompt",
                "You are a concise, helpful multi-party receptionist. Keep replies to 1–3 sentences. "
            ...
            # Always address the addressee explicitly for clarity
            ...
            # Update per-speaker memory AFTER generating the reply
            ...
            try:
                ...
                try:
                    # drain frames that arrived while speaking
                    _drain_queue(q); stream.start()
                except Exception as e:
...
def parse_args():
    p = argparse.ArgumentParser(description="Realtime multi-speaker receptionist (callback VAD + speaker labels)")
